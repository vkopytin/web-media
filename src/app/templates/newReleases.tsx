...
import { AlbumsView, NewReleasesView } from '../views';
        ...
        {_.map(view.newReleases, (item: NewReleasesView['newReleases'][0]) => {
            return <li key={item.id()} className="stack__card">
                ...
                <a href="#" className="card_link"
                    onClick={() => { view.selectAlbumCommand.exec(view.currentAlbum === item ? null : item) }}
                >
                ...
                <div className="card__info-hover">
                    {view.isLiked(item) && <span className="card__like icon icon-star-filled"
                        onClick={() => view.unlikeAlbumCommand.exec(item)}
                    ></span>}
                    {view.isLiked(item) || <span className="card__like icon icon-star"
                        onClick={() => view.likeAlbumCommand.exec(item)}
                    ></span>}
    ...
    <ul className="stack playlists">
        {_.map(view.featuredPlaylists, (item: NewReleasesView['featuredPlaylists'][0]) => {
            return <li key={item.id()} className="stack__card">
                <div className="card__img" style={{ backgroundImage: `url(${item.thumbnailUrl()})` }}></div>
                <a href="#" className="card_link"
                    onClick={() => { view.selectPlaylistCommand.exec(view.currentPlaylist === item ? null : item) }}
                >
                    <div className="card__img--hover" style={{ backgroundImage: `url(${item.thumbnailUrl()})` }}>
                    </div>
                </a>
